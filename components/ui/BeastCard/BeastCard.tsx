...
const BeastCard: FC<FuncProps> = ({ beast }) => {
  const { transferBeast, loadingBeast } = useUser()
  const { user } = useAuth()
  ...
  return (
    <Container>
      ...
      <BeastAction>
        ...
        {user?.addr == "0x8e2eaf41e9e4ec61" ? (
          <div>I'm the Bestie and I'll keep this. Thanks!</div>
        ) : (
          <button
            onClick={() => transferBeast("0x8e2eaf41e9e4ec61", beast.beastID)}
          >
            Transfer to Bestie
          </button>
        )}
